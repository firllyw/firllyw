
# Hi, I'm Firly 👋 

**Full-Stack Developer | Technical Lead | Cloud Architect** *Building scalable systems and AI-augmented workflows from Jakarta, Indonesia.*

---

### 🚀 About Me
I am a seasoned software engineer with 9+ years of experience and 3 years in technical leadership. I specialize in building high-traffic microservices and managing full product lifecycles. Currently, I am:
* 🏢 **Founder** at **PT Arafat Solusi Digital**, providing high-level tech services and managed solutions.
* 🛠️ **Building SIDRA**, a gamified habit-tracking app with a skill-tree progression system.
* ☁️ **Certified GCP Professional**, focusing on Cloud Architecture and Development.
* 🤖 **AI Orchestrator**, leveraging tools like Cursor and Antigravity to push the boundaries of rapid prototyping.

### 🛠️ Tech Stack & Expertise
* **Languages:** Go (Golang), TypeScript, Node.js, JavaScript.
* **Cloud & Infra:** Google Cloud Platform (Certified), Docker, Microservices Architecture.
* **Frameworks:** React, Next.js, Express, Gin.
* **Interests:** System Design, AI-augmented coding, Automotive Tech, and Web3.

### 📈 Current Projects
- **SIDRA:** A mobile app designed to turn life goals into an RPG-style achievement system.
- **Digital Clinic EMR:** A specialized workflow for mining medical reviews and work certificates.
- **Asset Management Systems:** Developing robust internal tools for LPS and Pettycash management.

### 📫 Connect with me
* **LinkedIn:** [Your Profile Link]
* **Email:** [Your Professional Email]
* **Website:** [PT Arafat Solusi Digital Link]

---

### 📊 GitHub Stats
![Firly's GitHub stats](https://github-readme-stats.vercel.app/api?username=firllyw&show_icons=true&theme=tokyonight)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=firllyw&layout=compact&theme=tokyonight)
